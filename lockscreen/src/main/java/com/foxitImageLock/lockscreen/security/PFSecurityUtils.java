...
public class PFSecurityUtils implements IPFSecurityUtils {
    ...
    @Override
    public String encode(@NonNull Context context, String alias, String input,
    ...
    // from https://developer.android.com/reference/android/security/keystore/KeyGenParameterSpec
    // .html
    // from https://code.google.com/p/android/issues/detail?id=197719
    private Cipher getEncodeCipher(@NonNull Context context, String alias,
    ...
    private boolean generateKeyIfNecessary(@NonNull Context context, @NonNull KeyStore keyStore,
                                           String alias,
                                           boolean isAuthenticationRequired) {
        try {
            return keyStore.containsAlias(alias) || generateKey(context, alias,
    ...
    private boolean generateKey(Context context, String keystoreAlias,
                                boolean isAuthenticationRequired) {
    ...
    private void initEncodeCipher(Cipher cipher, String alias, KeyStore keyStore)
        ...
        try {
            ...
            final PublicKey unrestricted =
                    KeyFactory.getInstance(key.getAlgorithm()).generatePublic(
